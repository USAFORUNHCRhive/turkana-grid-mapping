...
"""
This script is used to define the FCN8 model used for training."""
...
class FCN8(nn.Module):
    ...
    def extract_features(self, x_input):
        ...
        Args:
            x_input (tensor): Input image tensor.

        Returns:
            Feature set: Set of features extracted from the input image at different layers of the model
        """
    ...
    def forward(self, x):
        """Forward pass for the FCN8 model.

        Args:
            x (tensor): Input image tensor.

        Returns:
            logits_upsampled (tensor) : Prediction mask with poles
        """
        self.resnet50_32s.eval()
        ...
        logits_16s += nn.functional.interpolate(
            logits_32s, size=logits_16s_spatial_dim, mode="bilinear", align_corners=True
        )

        logits_8s += nn.functional.interpolate(
            logits_16s, size=logits_8s_spatial_dim, mode="bilinear", align_corners=True
        )
